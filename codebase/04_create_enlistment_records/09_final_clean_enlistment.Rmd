...
rearrange cols slightly so residence state is before state and county FIPS
```{r}
enlistment_records_final <- enlistment_records_final %>%
  relocate(residence_state, .before = residence_county_fips) %>%
  relocate(residence_state_fips, .after = residence_state)
```
